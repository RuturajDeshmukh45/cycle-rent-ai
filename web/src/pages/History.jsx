          ...
          {viewMode === 'list' ? (
            <div className="space-y-3">
              {filteredRides.map((ride) => (
                <div key={ride.id} className="rounded-2xl p-4 flex items-center gap-4 border hover:border-[var(--accent)] transition-all bg-[var(--bg-card)]" style={{ borderColor: 'var(--border)' }}>
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: 'var(--bg-input)' }}>
                    {typeEmoji[ride.cycle?.cycle_type] || '🚲'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm" style={{ fontFamily: 'Space Grotesk', color: 'var(--text-primary)' }}>{ride.cycle?.name}</p>
                  ...
                  <tbody className="divide-y" style={{ borderColor: 'var(--border)' }}>
                    {paginatedRides.map((ride) => (
                      <tr key={ride.id} className="hover:bg-[var(--bg-input)]">
                        <td className="p-4 text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{ride.cycle?.name}</td>
