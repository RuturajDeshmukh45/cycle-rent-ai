...
const Booking = () => {
  ...
  if (cycle.status !== 'available') return (
    <div className="p-6 text-center py-20">
      ...
      <p className="font-semibold" style={{ color: 'var(--text-secondary)', fontFamily: 'Syne' }}>Cycle not available</p>
  ...
  return (
    <div className="p-6 max-w-lg mx-auto page-enter">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-sm mb-5 transition-colors" style={{ color: 'var(--text-secondary)', fontFamily: 'Syne' }}>
        <ChevronLeft size={16} /> Back
      </button>

      <h1 className="text-xl font-extrabold mb-5" style={{ fontFamily: 'Syne', color: 'var(--text-primary)' }}>Book Your Ride</h1>
      ...
      <div className="rounded-2xl p-4 mb-5 flex items-center gap-4" style={{ background: 'var(--gradient-card)', border: '1px solid var(--border)' }}>
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-sm" style={{ fontFamily: 'Syne', color: 'var(--text-primary)' }}>{cycle.name}</h2>
          <div className="flex items-center gap-1 text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
            <MapPin size={11} /> {cycle.location}
          </div>
          <div className="text-base font-extrabold mt-0.5" style={{ fontFamily: 'Syne', color: 'var(--accent)' }}>
